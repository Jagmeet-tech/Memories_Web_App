...
export const getPost = (id) => async(dispatch) => {  //thunk : fun. returns a fn.
    try {
        dispatch({type : START_LOADING });
        const {data} = await api.fetchPost(id);
        dispatch({type : FETCH_POST,payload : data});      //an action dispatched to save response data in redux state.
        dispatch({type : END_LOADING });
    } catch (error) {
        console.log(error.message);
    }
